...
export const logUpdate = (filePath) => {
  ...
  const reset = `\x1b[0m`;
  return console.log(
    `[${`\x1b[2m`}${new Date().toLocaleTimeString()}${`\x1b[0m`}] ${colors}File "${filePath}" updated.${reset}`,
  );
};
...
export const logError = (err) => {
  ...
  const reset = `\x1b[0m`;
  return console.log(
    `[${`\x1b[2m`}${new Date().toLocaleTimeString()}${`\x1b[0m`}] ${colors}${err}${reset}`,
  );
};
...
export const isProduction = () =>
  Boolean(~process.argv.indexOf('--production'));
