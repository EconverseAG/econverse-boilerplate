...
// PostCSS Configs
const PostCSS = [true && autoprefixer(), isProduction && cssnano()].filter(
  Boolean,
);
...
export const Bundle = (file) => {
  ...
  const options = {
    ...
    onwarn: (warning) =>
      // Ignore warnings
      warning.code === 'EMPTY_BUNDLE' || warning.code === 'THIS_IS_UNDEFINED'
        ? null
        : Utils.logError(`[${warning.code}] ${warning.message}`),
  };
  ...
  // Dest pipe
  Stream = Stream.pipe(
    gulp_rename((file) => (file.extname = file.extname.replace('ts', 'js'))),
  ).pipe(dest(distPath));
