...
export const Styles = (source, callback) => {
  ...
  if (isProduction) {
    ...
    scss = scss.pipe(
      gulp_postcss([true && autoprefixer(), true && cssnano()].filter(Boolean)),
    );
  }
  ...
  scss = scss
    .pipe(
      gulp_rename(
        (file) => (file.dirname = file.dirname.replace('scss', 'css')),
      ),
    )
...
export const Stylelint = (source, callback) => {
  return source
    .pipe(plumber())
    .pipe(
      gulp_stylelint({ reporters: [{ formatter: 'string', console: true }] }),
    )
    .on('error', () => (typeof callback === 'function' ? callback() : null))
    .on('finish', () => (typeof callback === 'function' ? callback() : null));
};
...
export const Assets = (source, callback) => {
  return source
    .pipe(plumber())
